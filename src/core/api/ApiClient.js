...
import HttpMethodNotSupportException from '../exception/HttpMethodNotSupportException';
...
class Http {
    ...
    http(method, url, data = {}, type = 'json') {
        ...
        const { api } = Config;
        const config = this.getHttpConfig(method, `${api.host}:${api.port}${url}`, data, type);
        return send(config);
    }
}
/* 客户端使用时放在这里 */
